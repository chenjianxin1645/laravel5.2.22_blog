...
@section('content')
    ...
    <div class="crumb_warp">
        ...
        <a href="{{url('admin/link/index')}}">友情链接管理</a> &raquo; 添加友情链接
    </div>
    <!--面包屑导航 结束-->
    ...
    <div class="result_wrap">
        @if(count($errors)>0)
            ...
            @endforeach
        @endif
        <form action="{{url('admin/link/store')}}" method="post">
            {{csrf_field()}}
            <table class="add_tab">
                <tbody>
                <tr>
                ...
                <tr>
                    <th>友情链接描述：</th>
                    <td>
                        <textarea name="link_desc" required>{{old('link_desc')}}</textarea>
                    </td>
                </tr>
                <tr>
                    <th>友情链接排序：</th>
                    <td>
                        <input type="text" name="link_order" value="{{old('link_order')}}">
                    </td>
                </tr>
                <tr>
                    <th>相关操作</th>
